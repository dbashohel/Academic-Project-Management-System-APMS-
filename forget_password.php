...
if (isset($_POST["forgot_submit"])) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        $update_query = "UPDATE tcr_registration SET reset_token = '$token' WHERE user_email = '$user_email'";
        $update_result = mysqli_query($con, $update_query);

        if ($update_result) {
            // Send an email with the reset link containing the token
            $reset_link = "https://project.raisaagrofood.ltd/reset_password.php?token=$token";
            $to = $user_email;
            $subject = "Password Reset Request";
            $message = "Dear User,\n\nPlease click the following link to reset your password:\n$reset_link\n\nIf you did not request this reset, please ignore this email.";
            $headers = "From: Project <no-reply@project.raisaagrofood.ltd> \r\n";
